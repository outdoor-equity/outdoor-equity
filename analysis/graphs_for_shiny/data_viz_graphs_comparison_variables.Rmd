...
test_plot_asian <- 
  ...
  labs(x = "Distance Traveled (miles)",
       y = "Percentage (%)",
       title = "Asian",
       subtitle = "Breakdown of Race of Home ZIP Codes \nvs. Distance Traveled \nfor Overnight Reservations in California") +
  theme_minimal() +
  theme(legend.position = "none",
        plot.background = element_rect("white"),
        panel.grid.major.y = element_blank()) +
...
## -- data wrangle -- ##
test_race_bins_white <- data_ridb_acs_2018 %>% 
  select(agency, admin_unit, park, customer_zip, asian, black, hispanic_latinx, 
         multiracial, native_american, other, pacific_islander, white,
         distance_traveled_m) %>% 
  mutate(distance_traveled_mi = distance_traveled_m * 0.000621371) %>% 
...
test_race_bins_breaks_white <- quantile(test_race_bins_white$race_percentage, 
                                         prob = seq(0, 1, length = 6), type = 5)

# test_2 <- test_race_bins %>% 
#   select(asian, black, hispanic_latinx, multiracial, native_american, 
#          other, pacific_islander, white, distance_traveled_mi) %>% 
#   pivot_longer(cols = 1:8,
#                names_to = "race",
#                values_to = "race_percentage") %>% 
#   mutate(race = str_replace(string = race,
...
ggplot(data = test_race_bins_white) +
  geom_boxplot(aes(x = factor(race_percent_bins),
                   y = distance_traveled_mi, 
                   fill = race),
               varwidth = TRUE, outlier.shape = NA) +
  scale_fill_manual(values = racial_group_colors) +
  scale_x_discrete(labels = c("1" = paste("0 -", round(test_race_bins_breaks_white[[2]], 2)), 
                              "2" = paste(round(test_race_bins_breaks_white[[2]], 2), "-", 
                                          round(test_race_bins_breaks_white[[3]], 2)), 
                              "3" = paste(round(test_race_bins_breaks_white[[3]], 2), "-", 
                                          round(test_race_bins_breaks_white[[4]], 2)),
                              "4" = paste(round(test_race_bins_breaks_white[[4]], 2), "-", 
                                          round(test_race_bins_breaks_white[[5]], 2)), 
                              "5" = paste(round(test_race_bins_breaks_white[[5]], 2), "-", 
                                          round(test_race_bins_breaks_white[[6]], 2)))) +
  # scale_y_continuous(limits = c(0, 100),
  #                    breaks = seq(0, 100, 10),
  #                    minor_breaks = seq(0, 100, 5),
  #                    sec.axis = dup_axis()) +
...
# plot
plot_race_distance_traveled_3 <- ggplot(data = data_plot_race_distance_traveled_3) +
  geom_col(aes(x = distance_traveled_mi,
               y = race_estimate,
               fill = race)) +
  facet_wrap(~race, scales = "free_x") +
  scale_fill_manual(values = racial_group_colors) +
...
ggplot(data_plot_race_distance_traveled_3, aes(distance_traveled_mi)) + 
  geom_histogram(
    aes(weights=race_estimate)
    ) + 
