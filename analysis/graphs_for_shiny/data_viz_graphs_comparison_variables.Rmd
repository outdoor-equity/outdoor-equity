...
##need to create function to be able to run for loop??
...
#   breaks <- data %>% 
#     group_by(distance_traveled_bins) %>% 
#     summarize(quartile = quantile(race_percentage)) %>% 
#     filter(quartile == max(quartile))
#   
#   print(
#     ggplot(data = data) +
...
#                                   "4" = "181 - 299 mi", 
#                                   "5" = "299 - 3,614 mi")) +
#       scale_y_continuous(limits = c(0, paste((breaks %>% 
#                                                filter(race == paste(races[[i]])))[[2]])
#                                     ),
#                          breaks = seq(0, paste(round(max(breaks$quartile), 2) * 100), 10),
#                          minor_breaks = seq(0, paste(round(max(breaks$quartile), 2) * 100), 5),
#                          sec.axis = dup_axis()) +
#       labs(x = "Distance Traveled (miles)",
#            y = "Percentage (%)",
#            title = "Asian",
