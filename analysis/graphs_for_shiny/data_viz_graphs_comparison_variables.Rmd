...
```{r}
plot_race_distance_traveled_2 <- ggplot(data = data_plot_race_distance_traveled) +
  geom_line(aes(x = factor(distance_traveled_bins),
                y = race_percentage, 
                group = race, col = race))  +
...
```{r}
plot_race_site_type_2 <- ggplot(data = data_plot_race_site_type) +
  geom_line(aes(x = aggregated_site_type,
                y = race_percentage,
                group = race, col = race))  +
  scale_color_manual(values = racial_group_colors)  +
  scale_x_discrete(limits = c("equestrian", "remote", "rv only", "rv or tent", "shelter", "tent only", "water"),
...
```{r}
plot_race_booking_window_2 <- ggplot(data = data_plot_race_booking_window) +
  geom_line(aes(x = factor(booking_window_bins),
               y = race_percentage,
               group = race, col = race),
           stat = "identity",
           position = "dodge")  +
...
```{r}
plot_race_daily_cost_2 <- ggplot(data = data_plot_race_daily_cost) +
  geom_line(aes(x = factor(daily_cost_per_visitor_bins),
               y = race_percentage,
...
```{r}
plot_race_length_of_stay_2 <- ggplot(data = data_plot_race_length_of_stay) +
  geom_line(aes(x = factor(length_of_stay_bins),
               y = race_percentage,
               group = race, col = race))  +
  scale_color_manual(values = racial_group_colors) +
  scale_x_discrete(labels = c("1" = "1 \nday", "2" = "1 - 2 \ndays", "3" = "2 \ndays", 
...
```{r}
plot_education_distance_traveled_2 <- ggplot(data = data_plot_education_distance_traveled) +
  geom_line(aes(x = factor(distance_traveled_bins),
               y = education_percentage,
               group = education, col = education))  +
...
plot_education_distance_traveled_2
```
...
```{r}
plot_education_site_type_2 <- ggplot(data = data_plot_education_site_type) +
  geom_line(aes(x = aggregated_site_type,
                y = education_percentage, 
                group = education, col = education))  +
  scale_color_viridis_d() +
...
```{r}
plot_education_booking_window_2 <- ggplot(data = data_plot_education_booking_window) +
  geom_line(aes(x = factor(booking_window_bins),
               y = education_percentage,
               group = education, col = education))  +
  scale_color_viridis_d() +
  ...
  theme_minimal() +
  theme(plot.background = element_rect("white"))
plot_education_booking_window_2
```
...
```{r}
plot_education_daily_cost_2 <- ggplot(data = data_plot_education_daily_cost) +
  geom_line(aes(x = factor(daily_cost_per_visitor_bins),
               y = education_percentage,
               group = education, col = education))  +
  scale_color_viridis_d() +
...
```{r}
## -- data wrangle -- ##
data_plot_transportation_distance_traveled_bins <- data_ridb_acs_2018 %>% 
  select(agency, regional_area, park, customer_zip, no_vehicle, distance_traveled_m) %>% 
  mutate(distance_traveled_mi = distance_traveled_m * 0.000621371) %>% 
  drop_na(distance_traveled_mi) %>% 
  mutate(distance_traveled_bins = ntile(distance_traveled_mi, 5)) 

data_plot_transportation_distance_traveled <- data_plot_transportation_distance_traveled_bins %>% 
  group_by(distance_traveled_bins) %>% 
  summarize(no_vehicle = (mean(no_vehicle, na.rm = TRUE) * 100)) %>% 
  pivot_longer(cols = 2,
               names_to = "transportation",
               values_to = "transportation_percentage") %>% 
  mutate(race = str_replace(string = transportation,
                            pattern = "no_vehicle",
                            replacement = "No Access to Personal Vehicle"))
  
distance_traveled_bin_breaks <- quantile(data_plot_transportation_distance_traveled_bins$distance_traveled_mi, 
                                         prob = seq(0, 1, length = 6), type = 5)
...
# plot for shiny
plot_transportation_distance_traveled <- ggplot(data = data_plot_transportation_distance_traveled) +
  geom_col(aes(x = factor(distance_traveled_bins),
               y = transportation_percentage),
           stat = "identity",
           position = "dodge",
           fill = "#440154")  +
  scale_x_discrete(labels = c("1" = "0 - 60.2", "2" = "60.2 - 116.9", "3" = "116.9 - 180.9", 
  ...
  labs(x = "Distance Traveled (miles)",
       y = "Percentage (%)",
       title = "Breakdown of Lack of Access to Personal Vehicle Home ZIP Codes \nvs. Distance Traveled",
       subtitle = "for Overnight Reservations in California") +
  theme_minimal() +
  theme(plot.background = element_rect("white"),
        panel.grid.major.x = element_blank(),
...
plot_transportation_distance_traveled
```
...
```{r}
## -- data wrangle -- ##
data_plot_transportation_site_type_bins <- data_ridb_acs_2018 %>% 
  select(agency, regional_area, park, customer_zip, no_vehicle, aggregated_site_type) %>% 
  group_by(aggregated_site_type) %>% 
  summarize(no_vehicle = (mean(no_vehicle, na.rm = TRUE) * 100)) %>% 
  pivot_longer(cols = 2,
               names_to = "transportation",
               values_to = "transportation_percentage") %>% 
  mutate(transportation = str_replace(string = transportation,
                            pattern = "no_vehicle",
                            replacement = "No Access to Personal Vehicle"))
...
# plot for shiny
plot_transportation_site_type <- ggplot(data = data_plot_transportation_site_type_bins) +
  geom_col(aes(x = aggregated_site_type,
               y = transportation_percentage),
           stat = "identity",
           position = "dodge",
...
plot_data <- c("data_plot_race_distance_traveled", "data_plot_race_site_type", 
               ...
               "data_plot_transportation_distance_traveled", "data_plot_transportation_site_type_bins", 
