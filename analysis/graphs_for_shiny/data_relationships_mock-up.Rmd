...
plot_race_site_type <- ggplot(data = data_plot_race_site_type, 
                              aes(x = percent_n,
                                  ...
                                  text = paste0("Of visits to ", str_to_lower(aggregated_site_type) %>% 
                                                  str_replace(string = ., pattern = "rv", replacement = "RV"), 
                                                " sites, ", scales::percent(percent_n, accuracy = 0.1),
...
plot_race_daily_cost <- ggplot(data = data_plot_race_daily_cost, 
  ...
  geom_point(aes(x = mean_daily_cost,
                 ...
                 text = paste0("People who live in ZIP codes with high ", 
                               str_to_title(race) %>% str_replace(string = ., pattern = "\\(S\\)", "\\(s\\)"), " populations ",
                             "<br>pay an estimate of ", scales::dollar(mean_daily_cost), " per person per day.")),
             width = .25,
             size = 4, shape = 21, stroke = 2) +
  scale_fill_manual(values = racial_group_colors) +
  scale_color_manual(values = racial_group_colors) +
...
plot_education_distance_traveled <- ggplot(data = data_plot_education_distance_traveled, 
  ...
  geom_point(aes(x = mean_distance_traveled_mi,
                 ...
                 text = paste("Those who live in ZIP codes with high rates of a <br>maximum education level of", 
                              education, "<br>travel an estimated distance of",
...
plot_education_site_type <- ggplot(data = data_plot_education_site_type, 
                              aes(x = percent_n,
                                  ...
                                  text = paste0("Of visits to ", aggregated_site_type, " sites, ", scales::percent(percent_n, accuracy = 0.1),
                                                ...
                                                "<br>people who live in ZIP codes with high rates of <br>a maximum education level of ", 
                                                education))) +
  geom_col(position = "stack") +
  scale_fill_manual(values = education_group_colors) +
  scale_x_continuous(labels = scales::percent) +
...
plot_education_booking_window <- ggplot(data = data_plot_education_booking_window, 
  ...
  geom_point(aes(x = mean_booking_window,
                 y = reorder(education, mean_booking_window),
                 color = education, fill = education,
                 text = paste("Those who live in ZIP codes with high rates <br>of a maximum education level of", 
                              education, "<br>make their reservations", round(mean_booking_window, 0), "days before their trip.")),
             width = .25,
             size = 4, shape = 21, stroke = 2) +
  scale_fill_manual(values = education_group_colors) +
  scale_color_manual(values = education_group_colors) +
  labs(x = paste("Average Number of Days from Order Date to Reservation Date"),
       y = "",
       title = "Average Number of Days in Advance <br>Reservation is Booked for Populations with Different Levels of Education") + 
  theme_minimal() +
  theme(plot.background = element_rect("white"),
        panel.grid.major.y = element_blank(),
        legend.position = "none")
...
plot_education_daily_cost <- ggplot(data = data_plot_education_daily_cost, 
  ...
  geom_point(aes(x = mean_daily_cost,
                 ...
                 color = education, fill = education,
                 text = paste("Those who live in ZIP codes with high rates <br>of a maximum education level of", 
                              education, "<br>pay an average of", scales::dollar(mean_daily_cost), "per day.")),
             width = .25,
             size = 4, shape = 21, stroke = 2) +
  scale_fill_manual(values = education_group_colors) +
  scale_color_manual(values = education_group_colors) +
...
plot_education_length_of_stay <- ggplot(data = data_plot_education_length_of_stay, 
  ...
  geom_point(aes(x = mean_length_of_stay,
                 ...
                 color = education, fill = education,
                 text = paste("Those who live in ZIP codes with high rates <br>of a maximum education level of", 
                              education, "<br>stay an average of", round(mean_length_of_stay, 0), "days.")),
             width = .25,
             size = 4, shape = 21, stroke = 2) +
  scale_fill_manual(values = education_group_colors) +
  scale_color_manual(values = education_group_colors) +
...
plot_language_site_type <- ggplot(data = data_plot_language_site_type, 
                              aes(x = percent_n,
                                  ...
                                  text = paste0("Of visits to ", aggregated_site_type, " sites, ", scales::percent(percent_n, accuracy = 0.1),
                                                ...
                                                str_to_lower(language) %>% str_replace(string = ., pattern = "english", replacement = "English")))) +
  geom_col(position = "stack") +
  scale_fill_manual(values = language_group_colors) +
  scale_x_continuous(labels = scales::percent) +
  labs(x = paste("Percent of Total Reservations per Site Type Group"),
...
plot_language_daily_cost <- ggplot(data = data_plot_language_daily_cost, 
  ...
  geom_point(aes(x = mean_daily_cost,
                 ...
                 text = paste("Those who live in ZIP codes with high rates of <br>populations that", 
                              str_to_lower(language) %>% 
                                str_replace(string = ., pattern = "english", replacement = "English"), 
                              "<br>pay an estimated distance of", scales::dollar(mean_daily_cost), "per day.")),
             width = .25,
             size = 4, shape = 21, stroke = 2) +
  scale_fill_manual(values = language_group_colors) +
  scale_color_manual(values = language_group_colors) +
...
plot_language_length_of_stay <- ggplot(data = data_plot_language_length_of_stay, 
  ...
  geom_point(aes(x = mean_length_of_stay,
                 ...
                 text = paste("Those who live in ZIP codes with high rates of <br>populations that", 
                              str_to_lower(language) %>% 
                                str_replace(string = ., pattern = "english", replacement = "English"), 
                              "<br>stay an estimated distance of", round(mean_length_of_stay, 0), "days.")),
             width = .25,
             size = 4, shape = 21, stroke = 2) +
  scale_fill_manual(values = language_group_colors) +
  scale_color_manual(values = language_group_colors) +
...
```{r}
## -- data wrangle -- ##
median_income_groups <- c("english_only", "not_english_only")

data_plot_median_income_distance_traveled <- data.frame()

# create plot df for each group for RIDB data
for (i in seq_along(median_income_groups)){
  df_ca_bin_breaks <- data_ca_acs_2018 %>% 
    select(zip_code, english_only, not_english_only, mean_zip_code_population) %>% 
    pivot_longer(cols = 2:3,
                 names_to = "median_income",
                 values_to = "median_income_percentage") %>% 
    filter(median_income == median_income_groups[[i]]) %>% ## BACK TO i
    drop_na(median_income_percentage) 
  
  weighted_half <- weighted.mean(x = df_ca_bin_breaks$median_income_percentage, w = df_ca_bin_breaks$mean_zip_code_population)
  
  df_ca_bin_breaks <- df_ca_bin_breaks %>% filter(median_income_percentage >= weighted_half)
  
  weighted_quartile <- weighted.mean(x = df_ca_bin_breaks$median_income_percentage, w = df_ca_bin_breaks$mean_zip_code_population)
  
  df_median_income_group_i_longer <- data_ridb_acs_2018 %>% 
    select(agency, admin_unit, park, customer_zip, english_only, not_english_only,
           distance_traveled_m) %>% 
    mutate(distance_traveled_mi = distance_traveled_m * 0.000621371) %>% 
    drop_na(distance_traveled_mi) %>% 
  ...
  max_median_income_group_ridb <- df_median_income_group_i_longer %>%
    summarize(max = max(median_income_percentage))
  
  df_median_income_i_group <- df_median_income_group_i_longer %>% 
    filter(median_income_percentage >= weighted_quartile) %>% 
    summarize(mean_distance_traveled_mi = mean(distance_traveled_mi)) %>% 
    mutate(median_income = paste0(median_income_groups[[i]])) %>%  ## BACK TO i
    relocate(median_income, .before = 1) %>% 
    mutate(median_income = str_replace_all(string = median_income,
                                       pattern = "_",
                                       replacement = " "),
           median_income = str_replace(string = median_income,
                                  pattern = "^english only$",
                                  replacement = "Speak Only English at Home"),
           median_income = str_replace(string = median_income,
                                  pattern = "^not english only$",
                                  replacement = "Speak median_income Other Than English at Home"),
           median_income = str_to_title(median_income))

  data_plot_median_income_distance_traveled <- rbind(data_plot_median_income_distance_traveled, df_median_income_i_group)
    
assign(paste0("top_quartile_ca_median_income_", median_income_groups[[i]]), 
       c(weighted_quartile), 
       envir = .GlobalEnv)
assign(paste0("max_median_income_ridb_", median_income_groups[[i]]), 
       data.frame(max_median_income_group_ridb), 
       envir = .GlobalEnv)
}
...
median_income_group_colors <- c("Speak Only English At Home" = "#66c2a5", "Speak median_income Other Than English At Home" = "#8da0cb")

plot_median_income_distance_traveled <- ggplot(data = data_plot_median_income_distance_traveled, 
                                      aes(x = mean_distance_traveled_mi,
                                          y = reorder(median_income, mean_distance_traveled_mi))) +
  geom_segment(aes(xend = 0, yend = reorder(median_income, mean_distance_traveled_mi))) +
  geom_point(aes(x = mean_distance_traveled_mi,
                 y = reorder(median_income, mean_distance_traveled_mi),
                 color = median_income, fill = median_income,
                 text = paste("Those who live in ZIP codes with high rates of <br>populations that", 
                              str_to_lower(median_income) %>% 
                                str_replace(string = ., pattern = "english", replacement = "English"), 
                              "<br>travel an estimated distance of", round(mean_distance_traveled_mi, 0), "miles.")),
             width = .25,
             size = 4, shape = 21, stroke = 2) +
  scale_fill_manual(values = median_income_group_colors) +
  scale_color_manual(values = median_income_group_colors) +
  labs(x = paste("Average Distance Traveled from Home to Site (miles)"),
       y = "",
       title = "Average Distance Traveled to Reservation for <br>Populations that Speak a Language Other than <br>English at Home") + 
  theme_minimal() +
...
ggplotly(plot_median_distance_traveled,
         tooltip = list("text"))
```
