...
for (i in seq_along(racial_groups)){
  df_ca_bin_breaks <- data_ca_acs_2018 %>% 
    select(hs_GED_or_below, some_college, collage, master_or_above, mean_zip_code_population) %>% 
    pivot_longer(cols = 2:9,
                 names_to = "education",
                 values_to = "education_percentage") %>% 
    filter(race == education_groups[[i]]) %>% ## BACK TO i
  ...
  df_education_group_i_longer <- data_ridb_acs_2018 %>% 
    select(agency, admin_unit, park, hs_GED_or_below, some_college, collage, master_or_above,
           distance_traveled_m) %>% 
    mutate(distance_traveled_mi = distance_traveled_m * 0.000621371) %>% 
    drop_na(distance_traveled_mi) %>% 
  pivot_longer(cols = 5:12,
  ...
  df_racial_i_group <- df_racial_group_i_longer %>% 
    ...
    mutate(education = str_replace(string = education,
                              pattern = "_",
                              replacement = " "),
           race = str_to_title(education),
           race = str_replace(string = education,
                              pattern = "Hs",
                              replacement = "HS"),
           race = str_replace(string = education,
                              pattern = "Ged",
  ...
  data_plot_education_distance_traveled <- rbind(data_plot_race_distance_traveled, df_racial_i_group)
    
assign(paste0("top_quartile_ca_education", education_groups[[i]]), 
       c(weighted_quartile), 
       envir = .GlobalEnv)
