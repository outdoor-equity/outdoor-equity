...
for (i in seq_along(racial_groups)){
  ...
  data_plot_race_daily_cost_per_visitor <- rbind(data_plot_race_daily_cost_per_visitor, df_racial_group_i)
...
for (i in seq_along(education_groups)){
  ...
  data_plot_education_distance_traveled <- rbind(data_plot_education_distance_traveled, df_education_i_group) %>% 
    mutate(education = factor(education, levels = c("HS, GED, or Below", "Some College", 
                                                    "College", "Master Or Above")))
...
edu_group_colors <- c("HS, GED, or Below" = "#a6cee3", "Some College"  = "#1f78b4", 
                      "College" = "#b2df8a", "Master Or Above" = "#33a02c")
...
plot_education_distance_traveled <- ggplot(data = data_plot_education_distance_traveled, 
  ...
  geom_point(aes(x = mean_distance_traveled_mi,
                 ...
                 text = paste("People who live in ZIP codes with a high rate of populations with an education level at", 
                              str_to_title(education) %>% 
                                str_replace(string = ., pattern = "Hs Ged Or", replacement = "HS, GED, or"), 
                             "<br>travel an estimated distance of", round(mean_distance_traveled_mi, 0), "miles.")),
             width = .25,
             size = 4, shape = 21, stroke = 2) +
  scale_fill_manual(values = edu_group_colors) +
  scale_color_manual(values = edu_group_colors) +
  labs(x = paste("Average Distance Traveled from Home to Site (miles)"),
       y = "",
...
print(paste0("Hispanic Latinx: ", round(top_quartile_ca_education_some_college, 4)*100, " - ", round(max_education_ridb_some_college, 4) *100, "%"))
             
print(paste0("Asian: ", round(top_quartile_ca_education_college, 4)*100, " - ", round(max_education_ridb_college, 4) *100, "%"))
             
print(paste0("Black: ", round(top_quartile_ca_education_master_or_above, 4)*100, " - ", round(max_education_ridb_master_or_above, 4) *100, "%"))
```
