...
for (i in seq_along(racial_groups)){
  ...
  bin_breaks <- quantile(df_ca_bin_breaks$race_percentage, prob = seq(0, 1, length = 4), type = 5)
  
  max_racial_group_ridb <- data_ridb_acs_2018 %>% 
  ...
  df_racial_group <- data_ridb_acs_2018 %>% 
    select(agency, admin_unit, park, customer_zip, asian, black, hispanic_latinx, 
           multiracial, native_american, other, pacific_islander, white,
           distance_traveled_m) %>% 
    mutate(distance_traveled_mi = distance_traveled_m * 0.000621371) %>% 
  ...
  pivot_longer(cols = 5:12,
    ...
    filter(race == racial_groups[[i]]) %>% ## BACK TO i
    drop_na(race_percentage) %>% 
    mutate(race_percentage_bins = case_when(race_percentage <= bin_breaks[[2]] ~ paste0("Low"),
                                            race_percentage > bin_breaks[[2]] & race_percentage <= bin_breaks[[3]] ~ paste0("Medium"),
                                            race_percentage > bin_breaks[[3]] ~ paste0("High")),
           race_percentage_bins = factor(race_percentage_bins, levels = c("Low", "Medium", "High"))) %>% 
    group_by(race_percentage_bins) %>% 
    summarize(mean_distance_traveled_mi = mean(distance_traveled_mi)) %>% 
    mutate(race = paste0(racial_groups[[i]])) %>%  ## BACK TO 
  ...
  data_plot_race_distance_traveled <- rbind(data_plot_race_distance_traveled, df_racial_group)
    
assign(paste0("bin_breaks_", racial_groups[[i]]), 
       c(bin_breaks), 
       envir = .GlobalEnv)
...
# plot for shiny
plot_race_distance_traveled <- ggplot(data = data_plot_race_distance_traveled %>% filter(race_percentage_bins == "High")
                                      ) +
  geom_linerange(aes(x = race_percentage_bins,
                     ymin = 0,
                     ymax = mean_distance_traveled_mi,
                     color = reorder(race, mean_distance_traveled_mi)),
                 position = position_dodge(width = 1)#,
                 #width = .25
                 ) +
  geom_point(aes(x = race_percentage_bins,
                 y = mean_distance_traveled_mi,
                 color = reorder(race, mean_distance_traveled_mi), fill = reorder(race, mean_distance_traveled_mi),
                 text = paste("People who live in ZIP codes with", str_to_upper(race_percentage_bins), "Asian populations",
  ...
  labs(y = paste("Average Distance Traveled from Home to Site (miles)"),
       x = "",
       #color = "Race",
       #fill = "Race",
       title = "Average Distance Traveled to Reservation Based on <br>Different Racial Populations in ZIP code")
...
plot_asian <- ggplot(data = data_plot_race_distance_traveled_asian,
  ...
  geom_point(size = 5, color = "#009E73", 
             ...
             aes(text = paste("People who live in ZIP codes with", str_to_upper(race_percentage_bins), "Asian populations",
                             "<br>travel an estimated distance of", round(mean_distance_traveled_mi, 0), "miles."))) +
  labs(x = paste("Distance Traveled from Home to Site (miles)", "\U2192"),
       y = "",
       title = "Distance Traveled to Reservation Based on <br>Presence of Asians in ZIP code")

# black "#F0E442"

# hispanic_latinx "#CC79A7"

# multiracial "#56B4E9"

# native_american "#D55E00"

# other "#999999"

# pacific_islander "#E69F00"
