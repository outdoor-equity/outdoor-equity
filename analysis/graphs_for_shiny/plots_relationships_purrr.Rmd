...
function_highCutoff <- function(race_group, acs_df = data_ca_acs_2018){
...
```{r}
function_plot_race_siteType <- function(type_string){
...
```{r}
test <- data.frame(type_string = c("equestrian", "remote", "rv only", "rv or tent", "shelter", "tent only", "water")) 

test %>% map_dfr(function_plot_race_siteType)
```
