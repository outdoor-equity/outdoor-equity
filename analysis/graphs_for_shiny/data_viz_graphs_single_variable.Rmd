...
# plot for shiny app
plot_booking_window <-
  ggplot(data = data_plot_boooking_window) +
...
```{r}
## -- data wrangle -- ##
data_plot_daily_cost_per_visitor <- data_ridb_acs_2018 %>% 
...
# plot for shiny app
plot_daily_cost_per_visitor <-
  ggplot(data = data_plot_daily_cost_per_visitor) +
...
# plot for shiny app
plot_length_of_stay <-
  ggplot(data = data_plot_length_of_stay) +
...
data_plot_site_type <- data_ridb_acs_2018 %>% 
  ...
  mutate(aggregated_site_type = factor(aggregated_site_type, 
                                       levels = c("shelter", "rv only", "rv or tent",
                                                  ...
                                                  "water")))
data_plot_site_type$aggregated_site_type <- with(data_plot_site_type, reorder(aggregated_site_type, count))
...
data_plot_race_ridb$race <- with(data_plot_race_ridb, reorder(race, race_percent_average))

# CA population
data_plot_race_ca <- data_acs_2018_race_percent_California %>%
  summarize(white = (weighted.mean(white, zip_code_population, 
                                   na.rm = TRUE) * 100),
  ...
  mutate(race = str_replace(string = race,
                            ...
                            replacement = " "),
         race = str_to_title(race))
data_plot_race_ca$race <- with(data_plot_race_ca, reorder(race, race_percent_average))
...
# CA population
data_plot_education_ca <- data_acs_2018_education_percent_California %>%
  summarize(hs_GED_or_below = (weighted.mean(hs_GED_or_below, zip_code_population, 
                                             na.rm = TRUE) * 100),
            some_college = (weighted.mean(some_college, zip_code_population, 
                                          na.rm = TRUE) * 100),
            college = (weighted.mean(college, zip_code_population, 
                                     na.rm = TRUE) * 100),
            master_or_above = (weighted.mean(master_or_above, zip_code_population, 
                                             na.rm = TRUE) * 100))  %>%
  pivot_longer(cols = 1:4, names_to = "education", values_to = "education_percent_average") %>% 
  mutate(education = str_replace(string = education,
                            pattern = "hs_GED_or_below",
...
data_plot_language_ridb <- data_ridb_acs_2018 %>%
  select(agency, regional_area, park, customer_zip, not_english_only, english_only) %>% 
  mutate(not_english_only = not_english_only * 100,
         english_only = english_only * 100) %>% 
  drop_na(not_english_only, english_only)

# CA population
data_plot_language_ca <- data_acs_2020_language_percent_California %>%
...
# CA population
data_plot_median_income_ca <- data_acs_2018_median_income_California %>%
...
plot_data <- c("data_plot_distance_traveled", "data_plot_boooking_window", 
               "data_plot_daily_cost_per_visitor", "data_plot_length_of_stay", 
