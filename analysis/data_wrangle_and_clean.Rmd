...
# Create dataframes for "high" threshhold values for each ACS variables
...
data_race_quants <-
  race_group %>%
  map_dbl(race_top_quartile, acs_df = acs_ca_all) %>%
...
data_education_quants <-
  education_group %>%
  map_dbl(education_top_quartile, acs_df = acs_ca_all) %>%
...
data_language_quants <-
  language_group %>%
  map_dbl(language_top_quartile, acs_df = acs_ca_all) %>%
...
median_income_decile_list <-
  median_income_deciles(acs_df = acs_ca_all) %>%
  as.list()
```


# Create dataframe of all reservations that fall into a high category for each ACS variable
...
# Create dataframe for CA citie labels on visitorshed map
...
saveRDS(object = ca_city_labels,
        file = "../shiny/outdoor-equity-app/oe_app/data/ca_cities_geometries.rds")
```
