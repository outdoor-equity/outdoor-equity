...
#' Education x Daily Cost per Visitor Plotly
#'
...
#'
#' @return Plotly of educational categories compared to daily cost per visitor
#'
#' @examples
  ...
  function(admin_unitInput, 
    ...
    rdf <- reactive ({
      ...
      education_top_quartile_df %>%
        ...
        drop_na(daily_cost_per_visitor, education_percentage) %>% 
    ...
    plotly <- ggplot(data = rdf(), 
      ...
      geom_point(aes(color = education_y_lab, fill = education_y_lab,
                     text = paste0(comma(count, accuracy = 1), 
                                   ...
                                   "<br>",
                                   "paid between ", dollar(quartile_lower), " and ", dollar(quartile_upper), 
                                   " per day per person, with a median of ",dollar(median_daily_cost_per_visitor), 
                                   ".")),
                 size = 3.5, 
                 shape = 21, stroke = 2) +
