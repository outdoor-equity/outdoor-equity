...
plot_bar_race <- ggplot(data = data_plot_bar_race) +
  ...
  labs(x = "Average proportion of race in home ZIP codes",
       y = "Race",
       fill = "Race",
       title = "Racial Breakdown of Visitors' Home ZIP Codes for Reservations \nat Reservable Overnight Sites in California in 2018") +
  theme_minimal() +
...
```{r, echo=FALSE}
plot_bar_race / plot_bar_race_ca
```

The graphs below...

```{r, echo=FALSE}
plot_bar_race / plot_bar_race_us
```
...
data_plot_difference_race <- left_join(x = data_plot_bar_race, 
  ...
  mutate(percent_diff_ridb_ca = percent_ca - percent_ridb,
         percent_diff_ridb_us = percent_us - percent_ridb)
...
plot_bar_race_diff_ridb_ca <- ggplot(data = data_plot_difference_race) +
  geom_col(aes(x = percent_diff_ridb_ca,
               y = race, 
               fill = race)) +
  labs(x = "Difference in average proportion of race in home ZIP codes (CA - RIDB)",
       y = "Race",
       fill = "Race",
       title = "Difference in Racial Breakdown of Home ZIP Codes \nfor California Residents Compared to Visitors to \nReservable Overnight Sites in California in 2018") +
  theme_minimal() +
  theme(plot.background = element_rect("white"),
        #axis.text = element_text(size = 12),
        #axis.title = element_text(size = 14, face = "bold"),
...
plot_bar_race_diff_ridb_us <- ggplot(data = data_plot_difference_race) +
  geom_col(aes(x = percent_diff_ridb_us,
               y = race, 
               fill = race)) +
  labs(x = "Difference in average proportion of race in home ZIP codes (US - RIDB)",
       y = "Race",
       fill = "Race",
       title = "Difference in Racial Breakdown of Home ZIP Codes \nfor United States Residents Compared to Visitors to \nReservable Overnight Sites in California in 2018") +
  theme_minimal() +
  theme(plot.background = element_rect("white"),
        #axis.text = element_text(size = 12),
        #axis.title = element_text(size = 14, face = "bold"),
