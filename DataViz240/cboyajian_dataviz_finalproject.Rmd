...
```{r, include=FALSE}
data_plot_bar_race_ridb_ca_us <- data_plot_bar_race %>% 
  left_join(y = data_plot_bar_race_ca,
            by = c("race"),
            suffix = c("_ridb", "_ca")) %>% 
  left_join(y = data_plot_bar_race_us,
            by = c("race")) %>% 
  rename(ridb = race_percent_average_ridb,
         ca = race_percent_average_ca,
         us = race_percent_average) %>% 
  pivot_longer(cols = 2:4,
               names_to = "data_source",
               values_to = "race_percent_average") %>% 
  mutate(data_source = factor(data_source, levels = c("ridb", "ca", "us")))
```
...
plot_col_race_ridb_ca_us <- 
  ggplot(data = data_plot_bar_race_ridb_ca_us) +
  geom_col(aes(x = race_percent_average,
               y = race,
               fill = data_source),
           position = "dodge") +
  scale_fill_manual(aes(direction = -1)) # breaking
