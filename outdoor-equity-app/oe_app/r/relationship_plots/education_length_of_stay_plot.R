...
education_length_of_stay_plot <- function(admin_unitInput, 
                                          siteInput,
  ...
  rdf <- reactive ({
    ...
    education_top_quartile_df %>%
      ...
      select(park, customer_zip, 
             ...
             length_of_stay) %>% 
      drop_na(length_of_stay, education_percentage) %>% 
  ...
  plotly <- ggplot(data = rdf(), 
    ...
    geom_point(aes(color = education_y_lab, fill = education_y_lab,
                   text = paste0(scales::comma(count, accuracy = 1), 
                                 " unique visits were made by people who live in ZIP codes with",
                                 "<br>",
                                 "high rates of ", education, " as the highest level of education.",
                                 "<br>",
                                 "Typically these visitor stay between ",
