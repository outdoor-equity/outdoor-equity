...
#'
#' @return Plotly of language categories compared to booking window
#'
#' @examples

language_booking_window_plot <- function(admin_unitInput, 
                                         siteInput,
  ...
  rdf <- reactive ({
    ...
    language_top_quartile_df %>%
      ...
      select(park, customer_zip, 
             ...
             booking_window) %>% 
      drop_na(booking_window, language_percentage) %>% 
  ...
  plotly <- ggplot(data = rdf(), 
    ...
    geom_point(aes(color = language_y_lab, fill = language_y_lab,
                   text = paste0(scales::comma(count, accuracy = 1), 
                                 ...
                                 language, 
                                 ". Typically these visitors<br>reserved their visit between ",
  ...
  ggplotly(plotly,
    ...
    layout(title = list(text = paste0('<b>', siteInput, '<br>', admin_unitInput, '</b>',
                                      '<br>',
                                      'Number of Days in Advance Site is Reserved by Visitors with Different Home Languages'),
                        font = list(size = 15))) %>%  
