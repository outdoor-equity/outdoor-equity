...
agency_metadata_tables <- function(agency_df, title_as_string) {
  ...
  region_code <- 
    ...
    lapply(function(cell){ cell[!is.na(cell) & cell != ""]}) %>%  # remove NA values
    unlist() %>%                                                  # turn list into vector for adding to df
    sort(decreasing = FALSE)                                      # sort alphabetically
  region_description <- 
    list(unique(agency_df$region_description)) %>%                # create list of unique values
    lapply(function(cell){ cell[!is.na(cell) & cell != ""]}) %>%  # remove NA values
    unlist() %>%                                                  # turn list into vector for adding to df
    sort(decreasing = FALSE)                                      # sort alphabetically 
  parent_location <- 
    list(unique(agency_df$parent_location)) %>%                   # create list of unique values
    lapply(function(cell){ cell[!is.na(cell) & cell != ""]}) %>%  # remove NA values
    unlist() %>%                                                  # turn list into vector for adding to df
    sort(decreasing = FALSE)                                      # sort alphabetically 
  inventory_type <- 
    list(unique(agency_df$inventory_type)) %>%                    # create list of unique values
    lapply(function(cell){ cell[!is.na(cell) & cell != ""]}) %>%  # remove NA values
    unlist() %>%                                                  # turn list into vector for adding to df
    sort(decreasing = FALSE)                                      # sort alphabetically
  site_type <- 
    list(unique(agency_df$site_type)) %>%                         # create list of unique values
    lapply(function(cell){ cell[!is.na(cell) & cell != ""]}) %>%  # remove NA values
    unlist() %>%                                                  # turn list into vector for adding to df
    sort(decreasing = FALSE)                                      # sort alphabetically
  facility_state <- 
    list(unique(agency_df$facility_state)) %>%                    # create list of unique values
    lapply(function(cell){ cell[!is.na(cell) & cell != ""]}) %>%  # remove NA values
    unlist() %>%                                                  # turn list into vector for adding to df
    sort(decreasing = FALSE)                                      # sort alphabetically
  ...
  df$region_code <- c(region_code, 
                      rep("", nrow(df) - length(region_code))) # add black cells to fill df
  df$region_description <- c(region_description, 
                             rep("", nrow(df) - length(region_description)))
  df$parent_location <- c(parent_location, 
                          rep("", nrow(df) - length(parent_location)))
  df$facility_state <- c(facility_state, 
                         rep("", nrow(df) - length(facility_state)))
  df$site_type <- c(site_type, 
                    rep("", nrow(df) - length(site_type)))
  df$inventory_type <- c(inventory_type, 
                         rep("", nrow(df) - length(inventory_type)))
  df <- df[-c(1)]
